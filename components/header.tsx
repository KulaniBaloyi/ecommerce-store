...
const Header = () => {
    ...
    if(!isMounted)return
  return (
    <header className={`${pathname=="/collections/shop"&&"text-white "} bg-white fixed bg-transparent top-0 left-0 right-0 z-[999] transition-all duration-500 ease-in-out flex justify-between items-center p-4 lg:px-10 lg:py-4 ${
  ...
  >
    ...
    <ul className="flex justify-end items-center gap-4 flex-[.2]">
      ...
      <li onClick={()=>route.push("/cart")} className="relative cursor-pointer">
        {cartItems.length>0&&<p className="bg-red-500 rounded-full aspect-square h-4 grid place-content-center text-white absolute text-[.5rem] font-[500] -right-2 -top-2">{total}</p>}

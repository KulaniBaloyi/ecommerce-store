...
const search = useSearchParams();
  ...
  const fetchResults = async () => {
    try {
      const res = await fetch(`${process.env.BASE_URL}=${searchQuery}`);
      if (res.ok) {
  ...
  const fetchNewQuery = async ()=>{
    try {
      const res = await fetch(`${process.env.BASE_URL}=${newQuery}`);
      if (res.ok) {
        const data = await res.json(); // Parse the response to JSON
        setResults(data)
  ...
  return (
    <div className="min-h-[100dvh] mt-20 w-[90%] mx-auto flex flex-col gap-5 p-5 py-10 items-center overflow-hidden">
        ...
        <svg onClick={()=>clear()} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5 cursor-pointer hover:text-red-500">
